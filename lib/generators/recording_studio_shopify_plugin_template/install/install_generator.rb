...
module RecordingStudioShopifyPluginTemplate
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      desc "Installs RecordingStudioShopifyPluginTemplate engine into your application"

      class_option(
        :mount_path,
        type: :string,
        default: "/recording_studio_shopify_plugin_template",
        desc: "Route prefix used when mounting the engine"
      )

      def mount_engine
        route %(mount RecordingStudioShopifyPluginTemplate::Engine, at: "#{options[:mount_path]}")
      end

      def copy_initializer
        template "recording_studio_shopify_plugin_template_initializer.rb",
                 "config/initializers/recording_studio_shopify_plugin_template.rb"
      end

      def add_yaml_config
        prompt = "Would you like to add `config/recording_studio_shopify_plugin_template.yml` " \
                 "for environment-specific settings? [y/N]"
        return unless yes?(prompt)

        template "recording_studio_shopify_plugin_template.yml",
                 "config/recording_studio_shopify_plugin_template.yml"
      end
      ...
      def add_tailwind_source
        ...
        if missing_lines.empty?
          say "Tailwind already configured to include RecordingStudioShopifyPluginTemplate and FlatPack sources.",
              :green
          return
        end
      ...
      def inject_tailwind_sources(tailwind_css_path, missing_lines)
        ...
        say "Added RecordingStudioShopifyPluginTemplate and FlatPack sources to Tailwind CSS configuration.", :green
        say "Run 'bin/rails tailwindcss:build' to rebuild your CSS.", :green
      end

      def formatted_tailwind_source_block(missing_lines)
        [
          "\n/* Include RecordingStudioShopifyPluginTemplate engine views for Tailwind CSS */",
      ...
      def tailwind_source_lines
        vendor_views = "../../vendor/bundle/**/recording_studio_shopify_plugin_template/app/views/**/*.erb"
        bundler_views = "../../../../../../usr/local/bundle/ruby/**/bundler/gems/" \
                        "recording_studio_shopify_plugin_template-*/app/views/**/*.erb"
        [
          %(@source "#{vendor_views}";),
          %(@source "#{bundler_views}";),
